                ...
                <div class="card-body p-3 p-md-4">
                    <form method="POST" action="{{ route('ventas.store') }}" id="formVenta">
                        ...
                        <div class="row g-3 mb-4 p-3 p-md-4 bg-secondary bg-opacity-10 rounded-4 border border-light border-opacity-10">
                            <div class="col-md-4">
                                <label class="form-label fw-bold small text-muted">Cliente <span class="text-danger">*</span></label>
                                <select class="form-select border-0 rounded-pill bg-secondary bg-opacity-25 text-body focus-ring-primary" name="id_cliente" required>
                            ...
                            <div class="col-md-3">
                                <label class="form-label fw-bold small text-muted">Fecha <span class="text-danger">*</span></label>
                                <input type="datetime-local" class="form-control border-0 rounded-pill bg-secondary bg-opacity-25 text-body focus-ring-primary" name="fecha" value="{{ now()->format('Y-m-d\\TH:i') }}" required>
                                @error('fecha')<div class="text-danger small ms-2 mt-1">{{ $message }}</div>@enderror
                            </div>
                            <div class="col-md-2">
                                <label class="form-label fw-bold small text-muted">Impuesto ({{ $configuracion['moneda'] ?? '$' }} )</label>
                                <input type="number" step="0.01" class="form-control border-0 rounded-pill bg-secondary bg-opacity-25 text-body focus-ring-primary" name="impuesto" id="impuesto" value="0" oninput="calcularTotales()">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select border-0 rounded-pill bg-secondary bg-opacity-25 text-body focus-ring-primary" name="metodo_pago_id" id="metodo_pago_id" required>
                            ...
                            <div class="col-12 d-none" id="pagoDetallesWrap">
                                <div class="p-3 bg-secondary bg-opacity-10 rounded-4 border border-light border-opacity-10 shadow-sm">
                                    <div class="fw-bold text-body mb-3"><i class="bi bi-wallet2 me-2 text-primary"></i>Datos de pago</div>

                                    <div id="pagoEfectivoFields" class="d-none">
                                        <label class="form-label fw-bold small text-muted">Monto Recibido ({{ $configuracion['moneda'] ?? '$' }} )</label>
                                        <div class="input-group">
                                            <span class="input-group-text border-0 bg-secondary bg-opacity-25 text-muted">{{ $configuracion['moneda'] ?? '$' }} </span>
                                            <input type="number" step="0.01" class="form-control border-0 bg-secondary bg-opacity-25 text-body" name="monto_recibido" id="monto_recibido" placeholder="0.00">
                                        </div>
                                    ...
                                    <div id="pagoTransferenciaFields" class="d-none">
                                        ...
                                        <input type="text" class="form-control border-0 bg-secondary bg-opacity-25 text-body" name="referencia_pago" id="referencia_pago" maxlength="50" placeholder="Ej: 00123456">
                                        @error('referencia_pago')<div class="text-danger small ms-2 mt-1">{{ $message }}</div>@enderror
                                    </div>

                                    <div id="pagoTarjetaFields" class="d-none">
                                        <div class="mb-3">
                                            <label class="form-label fw-bold small text-muted">Referencia / Nro. Operación</label>
                                            <input type="text" class="form-control border-0 bg-secondary bg-opacity-25 text-body" name="referencia_pago" id="referencia_pago_tarjeta" maxlength="50" placeholder="Ej: 00123456">
                                            @error('referencia_pago')<div class="text-danger small ms-2 mt-1">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="mb-1">
                                            <label class="form-label fw-bold small text-muted">Últimos 4 dígitos</label>
                                            <input type="text" class="form-control border-0 bg-secondary bg-opacity-25 text-body" name="ultimos_digitos" id="ultimos_digitos" maxlength="4" placeholder="Ej: 4242">
                        ...
                        <div class="table-responsive mb-4 rounded-4 overflow-hidden shadow-sm border border-light border-opacity-10">
                            <table class="table align-middle mb-0 text-body" id="tablaDetalles">
                            ...
                            <div class="col-md-5 col-lg-4">
                                <div class="glass-card rounded-4 p-4">
                                    <div class="d-flex justify-content-between mb-2">
                                        ...
                                        <span class="fw-bold fs-5 text-body">{{ $configuracion['moneda'] ?? '$' }} <span id="lblSubtotal">0.00</span></span>
                                    </div>
                                    
                                    <div class="mb-3 border-bottom border-light border-opacity-10 pb-3">
                                        <label class="form-label fw-bold small text-muted">Descuento Global ({{ $configuracion['moneda'] ?? '$' }} )</label>
                                        <div class="input-group input-group-sm">
                                            <span class="input-group-text border-0 bg-secondary bg-opacity-25 text-muted">{{ $configuracion['moneda'] ?? '$' }} </span>
                                            <input type="number" step="0.01" min="0" class="form-control border-0 bg-secondary bg-opacity-25 text-body" name="descuento" id="descuento" value="0" oninput="calcularTotales()">
                                        </div>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label fw-bold small text-muted">Cupón</label>
                                        <input type="text" class="form-control form-control-sm rounded-pill bg-secondary bg-opacity-25 text-body border-0" name="cupon" placeholder="Código">
                                    </div>
...
<template id="filaTemplate">
    <tr>
        <td class="ps-4 border-0">
            <select class="form-select border-0 bg-secondary bg-opacity-25 rounded-pill text-body focus-ring-primary producto-select" name="id_producto[]" onchange="productoSeleccionado(this)" required>
        ...
        <td class="border-0">
            <input type="number" class="form-control border-0 bg-secondary bg-opacity-25 rounded-pill text-body focus-ring-primary cantidad-input" name="cantidad[]" value="1" min="1" oninput="calcularFila(this)" required>
        </td>
        <td class="border-0">
            <div class="input-group input-group-sm">
                <span class="input-group-text border-0 bg-secondary bg-opacity-25 rounded-start-pill text-muted">{{ $configuracion['moneda'] ?? '$' }} </span>
                <input type="number" step="0.01" class="form-control border-0 bg-secondary bg-opacity-25 rounded-end-pill text-body focus-ring-primary precio-input" name="precio_unitario[]" value="0" min="0" oninput="calcularFila(this)" required>
            </div>
        </td>
        <td class="text-end pe-4 border-0 fw-bold subtotal-text text-body">{{ $configuracion['moneda'] ?? '$' }} 0.00</td>
        <td class="text-center border-0">
