        ...
        <div class="col-xl-10">
            ...
            <div class="row g-4">
                <div class="col-md-4">
                    <div class="glass-card overflow-hidden h-100">
                        <div class="card-header bg-primary bg-opacity-10 border-bottom border-light border-opacity-10 py-3">
                            <h5 class="mb-0 fw-bold text-body"><i class="bi bi-info-circle me-2"></i>Resumen</h5>
                        </div>
                        <div class="card-body p-4">
                            <div class="mb-4">
                                ...
                                <div class="d-flex align-items-center p-3 bg-secondary bg-opacity-10 rounded-4 border border-light border-opacity-10">
                                    ...
                                    <p class="fw-bold text-body mb-0">{{ $compra->proveedor->nombre_empresa ?? '-' }}</p>
                                </div>
                            </div>
                            <div class="mb-4">
                                <label class="small text-muted text-uppercase fw-bold mb-1">Usuario</label>
                                <div class="d-flex align-items-center p-3 bg-secondary bg-opacity-10 rounded-4 border border-light border-opacity-10">
                                    ...
                                    <p class="fw-bold text-body mb-0">{{ $compra->usuario->nombre ?? '-' }}</p>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="small text-muted text-uppercase fw-bold mb-1">Fecha</label>
                                <p class="mb-0 fw-bold text-body"><i class="bi bi-calendar-event me-2 text-primary"></i>{{ $compra->fecha }}</p>
                            </div>
                            <hr class="my-4 border-light border-opacity-10">
                            <div class="d-flex justify-content-between mb-2 text-muted">
                                <span>Subtotal:</span>
                                <span class="fw-bold text-body">{{ number_format($compra->subtotal,2) }}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-2 text-muted">
                                <span>Impuesto:</span>
                                <span class="fw-bold text-body">{{ number_format($compra->impuesto,2) }}</span>
                            </div>
                ...
                <div class="col-md-8">
                    <div class="glass-card overflow-hidden h-100">
                        <div class="card-header bg-primary bg-opacity-10 border-bottom border-light border-opacity-10 py-3">
                            <h5 class="mb-0 fw-bold text-body"><i class="bi bi-box-seam me-2"></i>Productos</h5>
                        </div>
                                        ...
                                        <tr class="hover-bg-white-10 border-bottom border-light border-opacity-10">
                                            <td class="ps-4">
                                                <div class="d-flex align-items-center gap-3">
                                                    ...
                                                    @else
                                                        <div class="bg-secondary bg-opacity-10 rounded-3 d-flex align-items-center justify-content-center text-muted" style="width:48px;height:48px">
                                                            <i class="bi bi-image"></i>
                                                        </div>
                                                    @endif
                                                    <span class="fw-bold text-body">{{ $d->producto->nombre ?? 'N/D' }}</span>
                                                </div>
                                            </td>
                                            <td class="text-center">
                                                <span class="badge bg-secondary bg-opacity-10 text-body border border-light border-opacity-10 rounded-pill px-3">{{ $d->cantidad }}</span>
                                            </td>
                                            <td class="text-end text-muted fw-bold">{{ $configuracion['moneda'] ?? '$' }} {{ number_format($d->precio_compra,2) }}</td>
