    ...
    <div class="row justify-content-center">
        <div class="col-xl-10">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold mb-1 text-body"><i class="bi bi-receipt me-2 text-primary"></i>Venta #{{ $venta->id_venta }}</h4>
                    <p class="text-muted small mb-0">Detalle de la transacción</p>
                </div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-3">
                            <div class="p-3 bg-white bg-opacity-10 rounded-4 border border-light border-opacity-10 h-100">
                                <label class="small text-muted text-uppercase fw-bold mb-1">Fecha</label>
                                <p class="fw-bold text-body mb-0"><i class="bi bi-calendar3 me-2 text-primary"></i>{{ $venta->fecha }}</p>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="p-3 bg-white bg-opacity-10 rounded-4 border border-light border-opacity-10 h-100">
                                <label class="small text-muted text-uppercase fw-bold mb-1">Método de Pago</label>
                                <p class="fw-bold text-body mb-0"><i class="bi bi-credit-card me-2 text-primary"></i>{{ $venta->metodoPago->nombre ?? $venta->metodo_pago }}</p>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="p-3 bg-white bg-opacity-10 rounded-4 border border-light border-opacity-10 h-100">
                                ...
                                <div>
                                    <span class="badge rounded-pill px-3 py-1 {{ $venta->estado == 'COMPLETADO' ? 'bg-success bg-opacity-10 text-success border border-success' : 'bg-secondary bg-opacity-10 text-body border border-secondary' }} border-opacity-25">
            ...
            <div class="glass-card overflow-hidden">
                <div class="card-header bg-primary bg-opacity-10 border-bottom border-light border-opacity-10 py-3">
                    <h5 class="mb-0 fw-bold text-body"><i class="bi bi-basket me-2 text-primary"></i>Productos Adquiridos</h5>
                </div>
                <div class="card-body p-4">
                            ...
                            <tbody class="border-top-0">
                                ...
                                <tr class="hover-bg-white-10 transition-base">
                                    <td class="ps-4 border-bottom border-light border-opacity-10" style="width:80px">
                                        ...
                                        @else
                                            <div class="bg-white bg-opacity-10 rounded-3 d-flex align-items-center justify-content-center text-muted shadow-sm border border-light border-opacity-10" style="width:48px;height:48px">
                                                <i class="bi bi-image"></i>
                                            </div>
                                        @endif
                                    </td>
                                    <td class="border-bottom border-light border-opacity-10">
                                        <span class="fw-bold text-body">{{ $d->producto->nombre ?? 'Producto Eliminado' }}</span>
                                    </td>
                                    <td class="border-bottom border-light border-opacity-10">
                                        <span class="badge bg-white bg-opacity-10 text-body border border-light border-opacity-10 rounded-pill px-3">{{ $d->cantidad }}</span>
                                    </td>
                                    <td class="text-muted border-bottom border-light border-opacity-10">{{ $configuracion['moneda'] ?? '$' }} {{ number_format($d->precio_unitario,2) }}</td>
                                    <td class="text-end pe-4 fw-bold text-body border-bottom border-light border-opacity-10">{{ $configuracion['moneda'] ?? '$' }} {{ number_format($d->subtotal,2) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                            <tfoot class="bg-primary bg-opacity-10 text-body">
                                <tr>
