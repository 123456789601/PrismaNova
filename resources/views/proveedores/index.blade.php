...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1 text-body"><i class="bi bi-building me-2"></i>Proveedores</h4>
            <p class="text-muted small mb-0">Gestión de proveedores y socios</p>
        </div>
                ...
                <table class="table align-middle mb-0 text-body">
                    ...
                    <tbody class="border-top-0">
                        ...
                        <tr class="hover-bg-white-10 transition-all">
                            <td class="ps-4 fw-bold text-muted border-0">#{{ $p->id_proveedor }}</td>
                            <td class="border-0">
                                <div class="d-flex align-items-center">
                                    <div class="avatar-initial rounded-circle bg-secondary bg-opacity-10 text-body fw-bold me-3 d-flex justify-content-center align-items-center" style="width: 36px; height: 36px; font-size: 0.9rem;">
                                        {{ substr($p->nombre_empresa, 0, 1) }}
                                    </div>
                                    <span class="fw-bold text-body">{{ $p->nombre_empresa }}</span>
                                </div>
                            </td>
                            <td class="border-0"><span class="badge bg-secondary bg-opacity-10 text-body border border-secondary border-opacity-10 rounded-pill">{{ $p->nit }}</span></td>
                            <td class="border-0">
                                <div class="small text-body fw-medium">{{ $p->contacto }}</div>
                                @if($p->telefono)
                                <div class="small text-muted"><i class="bi bi-telephone me-1"></i>{{ $p->telefono }}</div>
                                @endif
                            </td>
                            <td class="border-0">
                                @if($p->estado == 'ACTIVO')
                                ...
                                @else
                                    <span class="badge bg-secondary bg-opacity-10 text-muted rounded-pill px-3 border border-secondary border-opacity-25">{{ $p->estado }}</span>
                                @endif
                            </td>
