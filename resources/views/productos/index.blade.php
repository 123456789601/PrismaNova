...
<div class="container-fluid py-4">
    <div class="glass-card overflow-hidden">
        <div class="card-header bg-transparent py-3 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center border-bottom border-light border-opacity-10 gap-3">
            <h4 class="mb-0 fw-bold text-body"><i class="bi bi-box-seam me-2 text-primary"></i>Productos</h4>
            ...
            <div class="px-4 py-3 border-bottom border-light border-opacity-10 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                <form class="d-flex flex-column flex-md-row gap-2 w-100 w-md-auto" method="GET" action="{{ route('productos.index') }}">
                    ...
                    <div class="input-group w-100 w-md-auto">
                        ...
                        <input type="text" name="q" value="{{ request('q') }}" class="form-control border-0 bg-secondary bg-opacity-10 text-body placeholder-light" placeholder="Buscar por nombre o código...">
                    ...
                    <tbody class="border-top-0">
                        ...
                        <tr class="hover-bg-white-10 transition-all">
                            <td class="ps-4 w-80-px border-bottom border-light border-opacity-10">
                                ...
                                @else
                                    <div class="bg-secondary bg-opacity-25 rounded-3 d-flex align-items-center justify-content-center text-muted small img-thumb-48 shadow-sm">
                                        <i class="bi bi-image"></i>
                                    </div>
                                @endif
                            </td>
                            <td class="fw-medium text-muted border-bottom border-light border-opacity-10">#{{ $p->id_producto }}</td>
                            <td class="fw-bold text-body border-bottom border-light border-opacity-10">{{ $p->nombre }}</td>
                            ...
                            <td class="small text-muted border-bottom border-light border-opacity-10">{{ $p->proveedor->nombre_empresa ?? '-' }}</td>
